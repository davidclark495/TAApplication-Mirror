...
class Slot extends PIXI.Graphics
{
    ...
    pointer_down()
    {
        ...
        this.availability_tracker.update_unsaved_warning(true);
    }
...
class AvailabilityTracker extends PIXI.Graphics
{
    ...
    constructor()
    {
        ...
        this.unsaved_warning, this.warning_text = this.draw_warning_label(false);
    ...
    build_grid()
    {
        ...
        // Week Days
        var label_container = new PIXI.Container();
        ...
        const tip_text = new PIXI.Text("Click and drag to set/un-set available times. (Darker slots are 'available')", {
            fontFamily: 'Arial',
            fontSize: 14,
            fill: this.text_color * 0xffffff,
            align: 'center',
        });
        tip_label.addChild(tip_text);
    ...
    draw_warning_label(has_unsaved_changes)
    {
        const num_days_with_buffer = 6;
        const num_slots_with_buffer = 56;
        const slot_width = AvailabilityTracker.width / num_days_with_buffer;
        const slot_height = AvailabilityTracker.height / num_slots_with_buffer;
        var unsaved_warning = new PIXI.Graphics();

        // Unsaved Changes WARNING
        unsaved_warning.x = slot_width * 5;
        unsaved_warning.y = slot_height * 4 * 13 + 8;
        var warning_text = new PIXI.Text("Unsaved changes", {
            fontFamily: 'Arial',
            fontSize: 14,
            fill: 0xff0000,
            align: 'center',
        });
        warning_text.visible = has_unsaved_changes;
        unsaved_warning.addChild(warning_text);
        //Draw rect
        if (!has_unsaved_changes) {
            unsaved_warning.beginFill(0xffffff);
            unsaved_warning.drawRect(0, 0, 200, 100);
        }



        this.app.stage.addChild(unsaved_warning)
        return unsaved_warning, warning_text;
    }

    update_unsaved_warning(has_unsaved_changes)
    {
        this.draw_warning_label(has_unsaved_changes);
        //this.unsaved_warning.visible = has_unsaved_changes;
    }
    ...
    build_slots()
    {
        ...
        for (let i = 0; i < 5 * 48; i++) {
            ...
            var new_slot = new Slot(this, i, false, x + x_gap / 2, y, slot_width - x_gap, slot_height);
            new_slot.draw_slot();
    ...
    load_slots()
    {
        ...
        $.ajax({
            ...
            .done((function (data)
            {
                $('#save_spinner').hide();
                if (data.length == 0) {
                    alert("Sorry, we couldn't find a schedule for this user.")
                    return data;
                }
                existing_slots = data;
    ...
    upload_slots()
    {
        ...
        $.ajax({
            ...
            .done(function (data)
            {
                ...
                this.update_unsaved_warning(false);
            }.bind(this))
            .fail(function (data)
            {
                console.log("Sorry, we could not save your schedule");
                return;
            })
